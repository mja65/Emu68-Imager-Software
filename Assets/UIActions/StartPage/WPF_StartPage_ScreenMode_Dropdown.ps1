...
$WPF_StartPage_ScreenMode_Dropdown.Add_SelectionChanged({
    ...
    $WBScreenModeDetails = ($Script:GUIActions.AvailableScreenModesWB | Where-Object {$_.FriendlyName -eq $Script:GUIActions.ScreenModetoUseWB})
    $WBScreenModeDefault = ($Script:GUIActions.AvailableScreenModesWB | Where-Object {$_.DefaultMode -eq $true -and $_.RTG -eq $false})
    ...
    If ($Script:GUIActions.ScreenModetoUseFriendlyName -eq 'Automatic'){
        ...
        if (([int]$PIScreenModeDetails.Width -lt [int]$WBScreenModeDetails.Width) -or ([int]$PIScreenModeDetails.Height -lt [int]$WBScreenModeDetails.Height)){
              ...
              elseif ($isRTG -eq $false){
                  $Script:GUIActions.ScreenModetoUseWB = $WBScreenModeDefault.FriendlyName   
